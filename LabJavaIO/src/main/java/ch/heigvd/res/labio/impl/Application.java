...
public class Application implements IApplication {
  ...
  void storeQuote(Quote quote, String filename) throws IOException {
    ...
    StringBuilder pathnameBuilder = new StringBuilder(WORKSPACE_DIRECTORY);
    for(String tag: quote.getTags()) {
      pathnameBuilder.append("/").append(tag);
    }

    File file = new File(pathnameBuilder.append("/").append(filename).toString());
    if(!file.getParentFile().mkdirs()) {
      throw new IOException("Store quote: Could not create parent directories");
    }

    if(!file.createNewFile()) {
      throw new IOException("Create new file: Could not create file");
    }

    BufferedOutputStream bos = new BufferedOutputStream(new FileOutputStream(file));
    bos.write(quote.getQuote().getBytes());
    bos.close();
    ...
    explorer.explore(new File(WORKSPACE_DIRECTORY), new IFileVisitor() {
      ...
      public void visit(File file) {
        ...
        try {
          ...
          System.err.println("Error writing absolute path");
        }
      }
    });
  }
